...
df_tthh   = uproot.open(indir+PRE+"_tthh_bbww_semi.root")["Delphes"].arrays(input_open,library="pd").sample(frac=1).reset_index(drop=True)
df_tth   = uproot.open(indir+PRE+"_tth.root")["Delphes"].arrays(input_open,library="pd").sample(frac=1).reset_index(drop=True)
df_ttzh   = uproot.open(indir+PRE+"_ttzh.root")["Delphes"].arrays(input_open,library="pd").sample(frac=1).reset_index(drop=True)
df_ttbbh   = uproot.open(indir+PRE+"_ttbbh.root")["Delphes"].arrays(input_open,library="pd").sample(frac=1).reset_index(drop=True)
df_ttvv   = uproot.open(indir+PRE+"_ttvv.root")["Delphes"].arrays(input_open,library="pd").sample(frac=1).reset_index(drop=True)
df_ttbbv   = uproot.open(indir+PRE+"_ttvv.root")["Delphes"].arrays(input_open,library="pd").sample(frac=1).reset_index(drop=True)
df_ttbb   = uproot.open(indir+PRE+"_ttbb.root")["Delphes"].arrays(input_open,library="pd").sample(frac=1).reset_index(drop=True)
df_tttt   = uproot.open(indir+PRE+"_tttt.root")["Delphes"].arrays(input_open,library="pd").sample(frac=1).reset_index(drop=True)
df_ttw   = uproot.open(indir+PRE+"_ttw.root")["Delphes"].arrays(input_open,library="pd").sample(frac=1).reset_index(drop=True)
...
print(f"n1, n2, n3, n4 = {n1} {n2} {n3} {n4} {n5} {n6}")
...
###################################################
# Load b-tagger model.
bfh_dir = "dnn_result/bfh_ss/best_model.h5" # model input
...
df_total["higgs_mass_list"] = df_total.apply(higgs_4C2, axis = 1) # MODIFY #
...
df_plot_G5 = df_total[(df_total["category"] == 4)]
df_plot_G6 = df_total[(df_total["category"] == 5)]
...
print("Plotting corr_matrix total"); plot_corrMatrix(df_total[input_dnn+input_3], outdir,"total")
#print("Plotting corr_matrix tthh"); plot_corrMatrix(df_tthh[input_dnn+input_3], outdir,"tthh")
#print("Plotting corr_matrix tthbb"); plot_corrMatrix(df_tthbb, outdir,"tthbb")
#print("Plotting corr_matrix ttbb"); plot_corrMatrix(df_ttbb[input_dnn+input_3], outdir,"ttbb")
...
plot_confusion_matrix(y_val, pred_val_arg, classes=process_names, normalize=True, cmap=plt.cm.PuRd, 
                    title='Normalized confusion matrix', savename=outdir+"/norm_confusion_matrix_val_PuRd.pdf")
plot_confusion_matrix(y_val, pred_val_arg, classes=process_names, normalize=True, cmap=plt.cm.spring, 
                    title='Normalized confusion matrix', savename=outdir+"/norm_confusion_matrix_val_spring.pdf")
plot_confusion_matrix(y_val, pred_val_arg, classes=process_names, normalize=True, cmap=plt.cm.cool, 
                    title='Normalized confusion matrix', savename=outdir+"/norm_confusion_matrix_val_cool.pdf")
