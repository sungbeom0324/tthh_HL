...
indir = "./skimmed/train/"
PRE = "ss2l"
outdir = "./dnn_result/" + PRE + "_4Cat"
os.makedirs(outdir, exist_ok=True)

log_path = os.path.join(outdir, "train_log.txt")
sys.stdout = TeeLogger(log_path)

process_names = ["ttHH", "ttbb", "ttw", "tttt"]
...
df["tth"]["category"] = 5; df["tth"]["process"] = 3
...
n_ttw  = len(df["ttw"])
n_ttzh = len(df["ttzh"])
n_ttbbh = len(df["ttbbh"])
n_ttbbv = len(df["ttbbv"])
n_ttvv = len(df["ttbbv"])
...
print("Others")
print(f"n_ttzh = {n_ttzh}")
print(f"n_ttbbh = {n_ttbbh}")
print(f"n_ttvv = {n_ttvv}")
print(f"n_ttbbv = {n_ttbbv}")


# 최소값 구해서 출력
ntrain = min(n_tthh, n_ttbb, n_tttt, n_ttw)
print("ntrain =", ntrain)
...
# ===========================
# Save Train ROOT Tree
# ===========================
print("# Saving Train ROOT Tree")

f_train = ROOT.TFile(outdir+"/ss2l_train.root", "RECREATE")
tree_train = ROOT.TTree("Delphes", "Train Tree")
...
tree_train.Branch('category', category_array, 'category/F')
tree_train.Branch('process', process_array, 'process/F')
tree_train.Branch('G1', G1_array, 'G1/F')
tree_train.Branch('G2', G2_array, 'G2/F')
tree_train.Branch('G3', G3_array, 'G3/F')
tree_train.Branch('G4', G4_array, 'G4/F')
tree_train.Branch('DNN', DNN_array, 'DNN/F')

for i in range(len(x_train)):
    category_array[0] = y_train[i][0]
    process_array[0] = process_train[i][0]
    G1_array[0] = pred_train[i][0]
    G2_array[0] = pred_train[i][1]
    G3_array[0] = pred_train[i][2]
    G4_array[0] = pred_train[i][3]
    DNN_array[0] = pred_train[i][0] / (pred_train[i][1]+pred_train[i][2]+pred_train[i][3]+1e-6)
    tree_train.Fill()
