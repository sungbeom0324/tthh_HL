...
process_names = ["ttHH", "ttbb", "tt Vector boson(s)", "tttt", "tt Single Higgs"]
...
df_tth["category"]   = 4; df_tth["process"]   = 3
df_ttbbv["category"] = 2; df_ttbbv["process"] = 4
df_tttt["category"]  = 3; df_tttt["process"]  = 5
df_ttbbh["category"] = 4; df_ttbbh["process"] = 6
df_ttvv["category"]  = 2; df_ttvv["process"]  = 7
df_ttzh["category"]  = 4; df_ttzh["process"]  = 8

# Undersampling으로 클래스 균형 맞추기
df_1 = pd.concat([df_tthh]).sample(frac=1).reset_index(drop=True)
df_2 = pd.concat([df_ttbb]).sample(frac=1).reset_index(drop=True)
df_3 = pd.concat([df_ttw, df_ttbbv, df_ttvv]).sample(frac=1).reset_index(drop=True)
df_4 = pd.concat([df_tttt]).sample(frac=1).reset_index(drop=True)
df_5 = pd.concat([df_tth, df_ttbbh, df_ttzh]).sample(frac=1).reset_index(drop=True)

n1, n2, n3, n4, n5 = len(df_1), len(df_2), len(df_3), len(df_4), len(df_5)
print(f"n1, n2, n3, n4, n5 = {n1} {n2} {n3} {n4} {n5}")
ntrain = min(n1, n2, n3, n4, n5)
...
df_val["DNN"] = df_val["G1"]/(df_val["G2"]+df_val["G3"]+df_val["G4"]+df_val["G5"]+1e-6)
