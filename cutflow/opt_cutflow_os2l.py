...
tthh = uproot.open(tthh)[Tree].arrays(input_open, library="pd"); weights["tthh"]=crossx["tthh"]/len(tthh)
tth = uproot.open(tth)[Tree].arrays(input_open, library="pd"); weights["tth"]=crossx["tth"]/len(tth)
ttbbh = uproot.open(ttbbh)[Tree].arrays(input_open, library="pd"); weights["ttbbh"]=crossx["ttbbh"]/len(ttbbh)
ttzh = uproot.open(ttzh)[Tree].arrays(input_open, library="pd"); weights["ttzh"]=crossx["ttzh"]/len(ttzh)
ttvv = uproot.open(ttvv)[Tree].arrays(input_open, library="pd"); weights["ttvv"]=crossx["ttvv"]/len(ttvv)
ttbbv = uproot.open(ttbbv)[Tree].arrays(input_open, library="pd"); weights["ttbbv"]=crossx["ttbbv"]/len(ttbbv)
ttbb = uproot.open(ttbb)[Tree].arrays(input_open, library="pd"); weights["ttbb"]=crossx["ttbb"]/len(ttbb)
ttbbbb = uproot.open(ttbbbb)[Tree].arrays(input_open, library="pd"); weights["ttbbbb"]=crossx["ttbbbb"]/len(ttbbbb)
tttt = uproot.open(tttt)[Tree].arrays(input_open, library="pd"); weights["tttt"]=crossx["tttt"]/len(tttt)

# Events selection and acceptance
def Acceptance(df, process):
    Accept = []
    S0 = len(df)
    df = df[df['Lep_size'] == 2]
    S1 = len(df)
...
ttbbh["category"]  = 1 ; ttbbh["process"]  = 3; ttbbh["name"] = "ttbbh"
ttvv["category"]   = 3 ; ttvv["process"]   = 4; ttvv["name"] = "ttvv"
ttbbv["category"]  = 1 ; ttbbv["process"]  = 5; ttbbv["name"] = "ttbbv"
...
#dnn_dir = "dnn_result/os2l_1116/best_model.h5"
dnn_dir = "dnn_result/os2l_1129/best_model.h5"
...
df_total["DNN"] = df_total["G1"]/(2*df_total["G2"]+df_total["G3"]+df_total["G4"]+1e-6)
...
df_score["DNN"] = df_score["G1"]/(2*df_score["G2"]+df_score["G3"]+df_score["G4"]+1e-6)

f = ROOT.TFile("os2l_score_1129.root", "RECREATE")
...
def Acceptance2(df_total, Acc, weights):
    thresholds = np.arange(0, 1.0, 0.01) # Scan best threshold
    best_sig = 0
    best_th = 0
...
with pd.ExcelWriter("os2l_cutflow_1129.xlsx") as writer: # modify #
    df_cutflow.to_excel(writer, sheet_name="Cutflow")
    df_acceptance.to_excel(writer, sheet_name="Acceptance")

print("Data has been written to os2l_cutflow.xlsx with two sheets: Cutflow and Acceptance")
