...
def Acceptance(df, df_name):
    Accept = []
    S0 = len(df)
    df = df[(df['bJet_size'] >= 3) & (df['Lep_size'] == 2) & (df['MET_E']>30)]
    S1 = len(df)
    df = df[df['SS_OS_DL'] == 1]
    S2 = len(df)
    df = df[df['j_ht'] > 300]
...
dnn_dir = "/home/stiger97/github/tthh_full_14TeV/dnn_result/ss2l/best_model.h5"
...
with pd.ExcelWriter("ss2l_cutflow.xlsx") as writer:
